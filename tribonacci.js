/*
it works basically like a Fibonacci, but summing the last 3 (instead of 2) numbers of the sequence to 
generate the next. 
...
//my solution submitted on Code Wars
function tribonacci(signature, n){
    ...
    if (n == 0){
        ...
        for (let i = 0; i < n-3; i++){
            tribArray.push(tribArray[tribArray.length-1] + 
            tribArray[tribArray.length-2] + 
            tribArray[tribArray.length - 3]);
        }
    }
    return tribArray;
}

//solution that made me facepalm:
//function tribonacci(signature,n){  
